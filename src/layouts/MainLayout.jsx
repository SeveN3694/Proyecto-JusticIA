...
export default function MainLayout() {
  ...
  const userRole = localStorage.getItem('userRole') || 'abogado';
  const isAuxiliar = userRole === 'auxiliar' || userRole === 'admin';
    ...
    <div className="flex h-screen bg-legal-dark text-neutral-200 overflow-hidden">
      ...
      <div className="w-20 flex flex-col items-center py-8 bg-legal-panel border-r border-legal-border shadow-2xl z-50 shrink-0">
        <img 
          ...
          alt="JusticIA Logo" 
          onClick={() => navigate(isAuxiliar ? '/digitalizacion' : '/dashboard')}
        />
          ...
          {!isAuxiliar && (
            <>
              ...
              >
                ...
                <span className="absolute left-14 bg-black text-xs px-2 py-1 rounded border border-legal-border opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50">JusticIA IA</span>
              </button>
        ...
        <button onClick={() => {
            ...
            navigate('/login');
          }} className="p-3 text-neutral-500 hover:text-red-400 hover:bg-red-400/10 rounded-xl transition-all mt-auto group relative border border-transparent">
